...
$id                                       = @$_POST['sfcm_sme_serv_formac_complemen_id']?: '';
$status                                   = 1;
$dt_cadastro                              = date("Y-m-d H:i:s");
$sme_servidor_id                          = strip_tags(@$_POST['sfcm_sme_servidor_id']?: '');
$sfcm_ue_compon_curricular_id             = @$_POST['sfcm_ue_compon_curricular_id']?: '';
$c_h                                      = @$_POST['sfcm_c_h']?: '';
$tableName      = 'sme_serv_formac_complemen';
$error          = false;
$result         = array();
$msg            = "";
try {
  ...
  foreach ($id as $kId => $vId) {
    if (!is_numeric($sfcm_ue_compon_curricular_id[$kId])) {
      $stmt = $db->prepare('
        DELETE 
          FROM '.$tableName.'
          WHERE id = ?');
      ...
      if (is_numeric($vId) && $vId != 0 ) {
        ...
        $stmt->bindValue(5, $sfcm_ue_compon_curricular_id[$kId]?: NULL);

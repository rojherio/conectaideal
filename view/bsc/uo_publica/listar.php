...
$stmt = $db->prepare("SELECT 
  ...
  up.bsc_pessoa_id,
  sp.nome AS setor_publico_nome,
  p.nome AS pessoa_nome
  FROM bsc_uo_publica AS up
  LEFT JOIN bsc_setor_publico AS sp ON sp.id = up.bsc_setor_publico_id
  LEFT JOIN bsc_pessoa AS p ON p.id = up.bsc_pessoa_id
  ORDER BY sp.nome");
$stmt->execute();
$rsRegistros = $stmt->fetchAll(PDO::FETCH_ASSOC);
//Consulta para DataTable - END
//Parámetros de títutlos - BEGIN
$tituloPagina             = "Listagem de Unidades Organizacionais/Órgãos Públicos(as)";
$descricaoPagina          = "Informações de unidades organizacionais/órgãos públicos(as)";
$tituloFormulario1        = "Tabela com listagem de Unidades Organizacionais/Órgãos Públicos(as)";
$descricaoFormulario1     = "Dados de identificação de unidades organizacionais/órgãos públicos(as)";
$tituloImpressao          = "Relatório de unidades organizacionais/órgãos públicos(as) cadastradas no sistema conectaideal";
    ...
    <div class="row m-1">
      <div class="col-12 ">
        ...
        <ul class="app-line-breadcrumbs mb-3">
          ...
          <li class="active">
            <a href="<?= PORTAL_URL; ?>" class="f-s-14 f-w-500">Unidades Organizacionais/Órgãos Públicos(as)</a>
          </li>
        </ul>
      </div>
    </div>
                  ...
                  <tbody>
                    ...
                    foreach ($rsRegistros as $kObj => $vObj) {
                      ...
                      <tr>
                        ...
                        <td id="td_bsc_setor_publico"><?= $vObj['setor_publico_nome']; ?></td>
                        <td id="td_bsc_pessoa_id"><?= $vObj['pessoa_nome']; ?></td>
...
?>
<script type="text/javascript" src="<?= PORTAL_URL; ?>control/bsc/uo_publica/listar.js"></script>
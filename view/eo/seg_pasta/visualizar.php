...
if (empty($id)) {
  header('Location: '.PORTAL_URL.'view/eo/seg_pasta/listar');
}
...
$stmt = $db->prepare("SELECT 
  ...
  FROM seg_pasta AS p
  LEFT JOIN seg_submodulo AS sm ON sm.id = seg_submodulo_id
  WHERE p.id = ? ;
  ORDER BY p.nome");
$stmt->bindValue(1, $id);
$stmt->execute();
    ...
    <div class="row m-1">
      <div class="col-12 ">
        ...
        <ul class="app-line-breadcrumbs mb-3">
          ...
          <li class="active">
            <a href="<?= PORTAL_URL; ?>" class="f-s-14 f-w-500">Pasta</a>
          </li>
        </ul>
      </div>
    </div>
            ...
            <div class="app-datatable-default overflow-auto">
              ...
              <table id="table_visualizar" class="table table-striped display app-data-table">
                ...
                <tbody>
                  <tr>
                    <td>SubModulo</td>
                    <td><?= $rsRegistro['seg_submodulo_nome']; ?></td>
                  </tr>
                  <tr>
                    <td>Nome</td>
...
?>
<script type="text/javascript" src="<?= PORTAL_URL; ?>control/eo/seg_pasta/visualizar.js"></script>
...
if (empty($id)) {
  header('Location: '.PORTAL_URL.'view/eo/seg_pagina/listar');
}
...
$stmt = $db->prepare("SELECT 
  ...
  pa.pasta,
  pa.caminho,
  pa.nome AS seg_pasta_nome
  FROM seg_pagina AS pa
  LEFT JOIN seg_pasta AS p ON p.id = seg_pasta_id
  WHERE pa.id = ? ;
  ORDER BY pa.nome");
$stmt->bindValue(1, $id);
$stmt->execute();
            ...
            <div class="app-datatable-default overflow-auto">
              ...
              <table id="table_visualizar" class="table table-striped display app-data-table">
                ...
                <tbody>
                  <tr>
                    <td>Pasta</td>
                    <td><?= $rsRegistro['seg_pasta_nome']; ?></td>
                  </tr>
                  <tr>
                    <td>Nome</td>
...
?>
<script type="text/javascript" src="<?= PORTAL_URL; ?>control/eo/seg_pagina/visualizar.js"></script>